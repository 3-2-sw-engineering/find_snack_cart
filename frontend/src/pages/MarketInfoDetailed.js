...
import { createComment } from "../shared/BackendRequests.js";
			...
			<div className="detailed">
				...
				>
					...
					>
						...
						<motion.button onClick={(e) => { stop(e); window.open(url, '_blank'); }} whileTap={{ y: 3 }}><div><BiNavigation size='20' color='#93BDF9' /></div>길찾기</motion.button>
						<motion.button onClick={(e) => { stop(e); }} whileTap={{ y: 3 }}><div><TiHeart size='20' color='#93BDF9' /></div>즐겨찾기</motion.button>
						<motion.button onClick={(e) => { stop(e); setShare(true); handleCopyClipBoard(url); }} whileTap={{ y: 3 }}><div><BsShare size='20' color='#93BDF9' /></div>공유하기</motion.button>
					</motion.div>
					<p>
						평점: 5.0 <br /> 영업시간: 10:00 ~ 20:00 <br /> 결제방법: {props.marketDetailed.market_payment_method.join('/')}
					</p>
				</motion.div>
				...
				>
					...
					<motion.button className='comment' onClick={(e) => { stop(e); writeReview(); }} whileTap={{ y: -3 }}>댓글 작성하기</motion.button>
				...
				</motion.div>
				{
					...
					<motion.div className='review' animate={{ y: "-236%" }}>
						...
						<p>{TextLen}/200</p>
						<motion.button className='review-confirm' onClick={(e) => { const now = Date.now(); stop(e); writeReview(); createComment(TextInput, starScore, 1,now,props.marketDetailed.market_index) }} whileTap={{ y: -3 }}>
						{//유저정보 넣어야 합니다.}
							완료
						</motion.button>
					</motion.div>
